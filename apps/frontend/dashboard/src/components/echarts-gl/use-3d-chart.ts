...
/**
 * 生成模拟 3D 饼图的配置项
 * @param pieData :饼图数据，格式为 [{name: 'xxx', value: 123}, {name: 'yyy', value: 456}]
 * @param internalDiameterRatio :内径比例，取值区间[0, 1)，默认为 1/3
  ...
  let baseOptions: any = {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        if (name !== 'mouseoutSeries') {
          return TooltipMarkerHTML(name, obj.value);
        }
        return '';
      },
    },
...
function bindListen(myChart: any, option: any) {
  ...
  myChart.on('mouseover', (params: any) => {
    ...
    if (hoveredIndex === params.seriesIndex) {
      ...
      if (hoveredIndex !== '') {
        ...
        let height = option.series[hoveredIndex].pieData.value;
      ...
      if (params.seriesName !== 'mouseoutSeries') {
        ...
        let height = option.series[params.seriesIndex].pieData.value;
