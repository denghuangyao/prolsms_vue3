import { checkSystemType, createViewsRoutes, preloadDynamicViews } from '@/utils'
import { DynamicViews } from '@/constants'
import type { RouteRecordRaw } from 'vue-router'
/** 路由页面在这里导入 */
async function _import(): Promise<RouteRecordRaw[]> {
  //1.根据系统类型筛选需要加载的模块路径
  const activePaths = Object.entries(DynamicViews)
    .filter(([systemType, _]) => checkSystemType(systemType))
    .map(([_, path]) => path)
