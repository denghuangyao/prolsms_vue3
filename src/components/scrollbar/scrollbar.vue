...
const handleScroll = (event: Event) => {
    ...
    console.log('handleScroll--', event, target, clientHeight, scrollHeight, scrollTop, isAtTop.value)
...
<template>
    <ScrollArea :class="cn(props.class)" :on-scroll="handleScroll">
        <div v-if="showShadowTop" class="scrollbar-top-shadow ease-in-out transition-opacity"
            :class="{ 'show': !isAtTop }"></div>
        <slot></slot>
        <div v-if="showShadowBottom" class="scrollbar-bottom-shadow ease-in-out"
